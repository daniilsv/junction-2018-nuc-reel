...
static void dump_temp_fill(struct net_buf_simple *msg)
{
	struct sensor_value val[2];
	get_hdc1010_val(val);
	s16_t tmp = sensor_value_to_double(&val[0]);
	net_buf_simple_add_le16(msg, tmp);
	tmp = sensor_value_to_double(&val[1]);
	net_buf_simple_add_le16(msg, tmp);
}

static void dump_light_fill(struct net_buf_simple *msg)
{
	struct sensor_value val[2];
...
static void dump_accel_fill(struct net_buf_simple *msg)
{
	...
	tmp = sensor_value_to_double(&val[1]);
	net_buf_simple_add_le16(msg, tmp);
	tmp = sensor_value_to_double(&val[2]);
	net_buf_simple_add_le16(msg, tmp);
}

static void sensor_dump(struct bt_mesh_model *model,
						struct bt_mesh_msg_ctx *ctx,
						struct net_buf_simple *buf)
{
	printk("dump\n");
...
static const struct bt_mesh_model_op sensor_srv_op[] = {
	...
	{BT_MESH_MODEL_OP_SENS_DUMP, 0, sensor_dump},
